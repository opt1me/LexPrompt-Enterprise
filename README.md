...
- **An export states the instant its dispositions were read, and that they can change.** A report is a snapshot of a mutable record: it carries the moment it was true, what each state was before, how many times it moved, and a line saying plainly that a disposition can be changed after an export is taken. The full per-review history is exportable on its own.
...
- **A card says who set the state it is showing, and when.** *"Rejected by R. Okafor, 16:04 — was Verified"*, with the full history one click away. A finding nobody has touched names nobody, which is a different fact from one somebody reset. A re-run's clearing reads as a re-run, not as a person un-verifying.
- **Somebody else's change arrives while you are looking at it**, attributed, with no reload — unless you are in the middle of composing a judgement about that same finding, in which case it is **held and announced** rather than swapped in under your hand.
- **You can see who else is here.** While a colleague has the review open, their initials appear in its header, and a marker appears on the clause they have selected. Presence is **approximate and advisory**: it is never stored, it expires within fifteen seconds of somebody's last heartbeat, it locks nothing and blocks no write. An absent name does not mean nobody is there. A face on a clause means somebody is *looking* at it and never that they have checked it.
- **You can ask a colleague to look at a clause**, with a message saying what you want them to look at. It reaches them in the review they open and in their own list of open requests, and either of you can close it. **It records no judgement** — asking someone to check something and deciding it are different acts, so they are two clicks and two records.
- **When this browser cannot reach the server it says so and stops offering to change anything.** The findings stay on screen; the controls that would write go dead, with a sentence saying why. What is on screen is never presented as current when it cannot be vouched for.

## Working across more than one matter

Four surfaces answer questions that are not about the review in front of you.

- **"Assigned to me", in the header.** How many open requests are addressed to you, across every matter in the firm, re-read from the server rather than counted up from pushes. It shows **nothing at all** when the answer is zero, and a **visible marker that is not a number** when it does not know — a failed read never renders as `0`, because a badge that quietly says "nothing" when it means "I could not ask" is how a colleague waits for something you were never told about. When it does know, its tooltip names the matters the requests are in.
- **An assignee chip, where the request is.** On the finding card, on the clause rail and in the grid cell: who was asked, by whom. It carries **no message** (a colleague's private brief is not a bystander's to read), it offers **no control**, and it is drawn in no state or outcome colour — being asked to look at a clause is not a judgement about it, and the chip must never be mistakable for one.
- **Firm-wide search** — `⌘K`/`Ctrl-K`, or the search control in the header. **It searches names, not documents**: matter names, clients and references, document and precedent names, review, collection and playbook names, and the clause titles in a playbook's published version. It does **not** search the text inside your documents, and it says so at the bottom of every result set, including an empty one. It also reports an outcome **for every source on every answer**, so "nothing matched" and "one part of the search broke" are different screens with different words and different controls — a partial answer is never shown as a complete one.
- **A Report tab, beside the cards and the grid.** The report the export already produces, on screen, before you download it. It is a third renderer over the same findings — it declares no wording of its own, so what you read there is what the DOCX and the CSV will say, including the instant the dispositions were read and the line saying they can still change.
...
- **The chrome is honest about what the app can actually do.** The header avatar shows **your own** initials, taken from the account you signed in with, and it is still the only avatar anywhere — it never becomes somebody else's. The three affordances this line used to say were missing have arrived, each with the mechanism underneath it: the **assignee chip** (an assignment table and a review-scoped read), the **"assigned to me" counter** (a cross-matter inbox route), and the **firm-wide search box** (one search route with a declared corpus). What is still absent is still absent for the same reason: there is **no firm tag**, no mobile "Assigned" tab, and no second person's name in the header, because no mechanism has arrived for any of them and an affordance that implies otherwise would be a promise the app cannot keep. The matter activity feed is a real, append-only record now, and it names the person who did each thing.
...
- **Postgres** — every record: matters, documents' metadata and extracted text, collections, reviews, **every finding as its own row along with the disposition a person gave it and the complete history of every change to that disposition**, playbooks and their published versions, changesets, precedent sets, standard-position evidence, and the user rows sign-in creates. Locally a container on a network with no route out; in a firm a Flexible Server with public network access disabled and a private endpoint.
...
**A review runs on the server, not in your browser.** Starting one creates a job: the browser asks for it and then watches it, and closing the tab or reloading the page does not stop it — reopen the review and it is still going, or has finished. If the service restarts mid-run the run picks up where it was rather than dying with the process. **Cancelling is not failing**: a run you stopped says so, keeps every finding it had already produced, and is distinguishable on screen from one that stopped on its own.

**A document is read by the server, and while it is being read it says so.** Adding a file stores its bytes and returns immediately; the text is extracted a moment later. Until that finishes the document shows *"Still being read"*, cannot be chosen as a review target, and the API refuses a review of it by name — because a review of a document whose text has not arrived reports every clause as absent, which reads as an answer. If the read fails, the document says why and offers to try again; the bytes are still there, so you do not have to remove it and add the same file back.
...
### Taking the audit record out as a file

An administrator can export this workspace's audited record from `/admin/audit` — the audit log, the disposition history and the run table, over a date range they choose.

**Every extract states what it covers.** Its workspace, the range, the instant it was taken, who took it, the time zone the instants are in, and **every source by name with its row count** — a source with no rows in the range is listed with a zero, because an omitted source reads as one that was never covered. The counts come from the rows the file actually contains, measured after they were fetched, not from a second query that could disagree with them.

**It refuses rather than truncating.** A range holding more rows in any one source than a single extract may carry is refused with the count and the source named, and a request to narrow the range. There is no version of this file whose rows stop at a ceiling nobody stated: an audit extract is read months later by somebody who was not there, and gaps in one are indistinguishable from absences of activity.

**A range is required.** There is no default and no "everything" button — the whole history is a decision somebody makes, not one this route makes for them.

**It does not say where a review was processed.** The `run` row records the provider, the model and the jurisdiction it ran under, but a *finding* carries no run id — a retry re-runs one clause under whatever the workspace's model choice is now — so no clause can honestly be attributed to a run, and the extract does not pretend otherwise. The gateway's own call log remains the record of what went where.
...
- **An account can be turned off, and a name can be retired — but nothing here erases a decision.** An administrator can disable a person's account from `/admin/people`; they are refused on their very next request, with the token they already hold, and signing in again does not undo it. An administrator can also **retire a person's name**: their display name and email address are replaced with a stable pseudonym (`Former user 3f2a1b0c`) and the account is turned off, in one row, permanently — LexPrompt cannot put the name back.

  **That is not deletion, and the screen does not call it deletion.** Every disposition, note, assignment and audited act that person recorded stays exactly as it is and stays attributed to them by their internal id: `finding_disposition_event` and `audit_event` are append-only by database grant, which is what makes them evidence rather than a claim, and the application role holds no `delete` on the person's own record either — deleting one would orphan every attribution they authored. So after retiring a name, a card that used to read *"Verified by P Leaver"* reads *"Verified by Former user 3f2a1b0c"*, and the fact that somebody verified it, when, and what they changed it from, is untouched.

  **What survives, precisely:** the person's row (with the pseudonym), their internal id on every record they authored, every disposition and its full history, every note, every assignment, and every audit row. **What goes:** the display name and the email address, everywhere they were stored, which is one row. Whether that satisfies a particular erasure request is a question for your firm's own data-protection adviser, and LexPrompt takes no position on it — what it will not do is offer a button that says "delete this person" over an implementation that cannot.
...
**Verified against the running stack without a browser:** the whole of `npm run test:pg` — 477 tests over a real Postgres, including every table's whole-record round trip, the grants, and the matter cascade deleting real blobs out of Azurite; real Keycloak tokens accepted and a user in no mapped group refused with no account row created; and `GET /api/v1/matters` with no token answering `401 sign_in_required` rather than an empty list, which is the founding rule holding at the HTTP boundary.

**Stage 4, with two real accounts and two real tokens, on the running stack:** the seeded
realm's `trainee` and `partner` sign in through a local-only test client (the app's own
client still has no direct-access grant, asserted); one person's disposition change
reaches the other's WebSocket, attributed, in **9–40 ms** through nginx and **2–30 ms
across two `api` replicas**; a stale change is refused `409` carrying the row that
replaced it, and the same change offered again against the new version writes a second
history row; a presence beat reaches another person's socket in **3 ms**, crosses replicas
in **2 ms**, clears within **4 ms** of a tab closing and **expires 15.7-19.4 s after the beats
stop** — measured against the shipped 15 s TTL in the running process, not a faked clock;
an assignment reaches its assignee's socket in **33 ms** and appears in their own open
list; and `audit_event` holds the assignment while holding no disposition change.

**Stage 5, with three real accounts — a trainee, a partner and an administrator:** every
route the design puts at the administrator bar, enumerated from the authorisation table
itself rather than typed out, is **refused for the trainee and for the partner and reached
by the administrator** — thirteen routes, three callers each, plus a fourth pass with no
token at all; a role mapping is previewed (writing nothing), created, changed and removed,
producing **three audit rows and no disposition row**; the trainee's role moves on their
**very next request with the token they already hold** and moves back when the mapping is
removed; the `api` container is **restarted for real** and an administrator's own mapping
survives it while a configuration row the deployment no longer names is revoked; a person
is disabled, refused on their next request, re-enabled, and a throwaway account is
**pseudonymised through the shipped route** with the disposition history fingerprinted
before and after and **identical**; the providers response is swept for every credential
this stack is configured with; an extract's manifest is compared **against its own rows**,
and an unbounded range is refused. The mutation that matters most was run against the
running stack: removing `and source = 'configuration'` from the startup seed, rebuilding
the image and restarting it, **erased the administrator's mapping** and reddened the named
test.

**And the engine, against the real containers:** `npm run test:compose` seeds a 150-cell run, restarts the `api` container while cells are leased, and waits for the run to finish — 150 done, none left `pending`, and the two state machines agreeing. That is the one claim about surviving a restart that no unit test can make.

**Never exercised against data a person made.** The database this was all run against has **no reviews in it at all**: `finding_migration_report` records *"Migrated 0 findings; 0 human-authored records censused"*. The machinery that turns a review's findings from one JSON blob into rows — the census, the shred, the key-by-key reconciliation and the freeze that follows them — has been driven only by fixtures. It is the part of this rebuild whose correctness matters most and the part with the least real evidence behind it, and the reason is the same one below: nobody has been able to drive a browser.

**Still not done, five stages on, and still the largest gap in this file. No credentials
were entered.** Stage 4's and Stage 5's screens are in exactly the same position as every
screen before them: the conflict notice, the held-update line, the stale banner and its
dead controls, the presence roster and its clause marker, the assign panel and the "asked
of you" panel — and now the "assigned to me" marker, the assignee chip, the search
palette's empty-versus-broken pair, the Report tab, and all four administration screens —
are covered by unit and component tests and by **nothing that has looked at a screen**.
Whether a presence face reads as *"somebody is looking at this"* rather than *"somebody
has checked this"*, whether a refusal reads as an error rather than as a decision, whether
the counter's "not known" marker reads as *not known* rather than as decoration, and
whether an administrator reads the sentence describing a widening before confirming it,
are questions only a person can answer. `docs/BROWSER-VERIFICATION.md` is the checklist
and it names every one of them.

**Not done, and this is the largest gap in this file. No credentials were entered.** **Nothing in the server rebuild past the sign-in redirect has ever been watched in a browser**, so the token exchange, an end-to-end review, the four seeded accounts seeing different things, the screen that moves this browser's data to the server, the sentence a lawyer reads when bringing in a precedent document, and the standard-position evidence panel are **all covered by unit, integration, real-Postgres and real-token HTTP tests, and by nothing that has looked at a screen.** Everything the server engine added is in the same position: a run started and then the tab reloaded, the findings filling in as cells finish, *"Still being read"* on a freshly added document and the moment it becomes reviewable, a verification made while a run is still going, a retry clearing a verification, and a cancel stopping the cells. Each is tested; none has been seen. Browser automation was unavailable throughout: the Chrome extension disconnected mid-way and Playwright's driver times out. That is stated rather than worked around, because two of this project's worst defects — a review screen showing zero documents, and a failed review becoming permanently unopenable — were invisible to thousands of passing tests and appeared only when somebody drove the real app. **These need a person.**
...
**`review.findings` is a frozen backup column.** Findings became rows in this release and the old JSON column was kept rather than dropped, on the rule that you never delete what you cannot read: it is the only thing that can still answer *"did the migration lose anything?"*, and the reconciliation tool that asks reads it. No application role can update it. A later release drops it once the rows are confirmed good — the same release that deletes this browser's local IndexedDB database.
...
- **Two people can now work on one review, and what each of them did is on the record.** A disposition can be changed by any reviewer at any time — a partner may override a trainee's verification, and it may be changed back — and **every change is recorded with who made it and when**, in an append-only table no application role may update or delete. A card names the person who set the state it is showing, and says what the state was before. A change made while you had the review open **arrives without a reload**, attributed. A change you submit against a version somebody has already moved is **refused, not merged**: the refusal names who changed it, to what, and when, and offers your change again against the row that won — one click, by you, never automatically. The two cross-matter surfaces have arrived since — an assignee chip on the card, the clause rail and the grid cell, and an "assigned to me" counter in the header. What has *not* arrived, and is not planned: **nothing notifies anybody outside the app**. An assignment reaches somebody who opens LexPrompt, and the panel where you make one says so in those words.
- **A user's role comes from their group membership at the identity provider, and there is now a screen for the half of the mapping an administrator owns.** `role_mapping` is two-sourced: rows that came from `API_ROLE_MAPPINGS` are **deployment configuration** and no request can write them — the database refuses the statement, not the handler — and rows an administrator typed at `/admin/roles` are theirs to add, change and remove. Every write is audited and states its effect in the server's own words before it happens, a widening asks for the role name to be typed, and a change that would leave the workspace with no administrator mapping at all is refused inside the transaction that would have made it. **There is still no per-person role**: what an administrator changes is a group's mapping, and `app_user.role` is a cache of what the last request resolved to, labelled as exactly that wherever it is shown.
- **Retention configuration is not on any screen.** §7 lists it among an administrator's powers and nothing implements it: `audit_event` is partitioned by month and its retention is a partition `DETACH` somebody has to schedule. A screen for a policy nobody has chosen would be a screen that chose one.
