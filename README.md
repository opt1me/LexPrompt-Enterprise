...
**Run starts and cancellations are recorded on the run's own row, not in an append-only audit log.** There is no `audit_event` table yet: what a run recorded is who asked for it, when it started, when it finished and how it ended, on a row that can be updated. Every change to a finding's **disposition** — the judgement a person made — *is* in an append-only table that no application role may update or delete, and that history is complete. But an audit export covering this period has to say where its run history came from, because the two have different guarantees. The log that unifies them arrives with the activity feed that reads it.
