...
import { Plus, Table, FileText, Download, X, MessageSquare, Loader, CheckCircle2, AlertCircle, Maximize2, Minimize2, Edit3, Eye, AlignLeft, Bot, Type } from 'lucide-react';
    ...
    const processQueue = async (colsToProcess: TabularColumn[]) => {
        ...
        documents.forEach(doc => {
            ...
            colsToProcess.forEach(col => {
                if (!nextData[doc.id][col.id]) {
                    nextData[doc.id][col.id] = { value: "", quote: "", confidence: "Low", status: "loading" };
                }
            });
        });
        setData(nextData);

        // Process sequentially to avoid rate limits (in production, use a queue)
                ...
                try {
                    ...
                    setData(prev => ({
                     ...
                     setData(prev => ({
                        ...
                        [doc.id]: {
                            ...prev[doc.id],
                            [col.id]: { value: "Error", quote: "", confidence: "Low", status: "error" }
                        }
                    }));
                }
            }
            ...
            <div className="flex-1 flex overflow-hidden relative">
                ...
                {selectedCell && activeDoc && activeCellData && (
                    <div className="w-[45%] border-l border-white/10 flex flex-col bg-[#0e0e0e] shadow-2xl z-20 absolute right-0 top-0 bottom-0 animate-in slide-in-from-right duration-300 h-full">
                        ...
                        {/* Editor Section */}
                        <div className="p-6 border-b border-white/10 bg-[#111] space-y-4 shrink-0 overflow-y-auto max-h-[40%] custom-scrollbar">
                            <div className="flex justify-between items-end">
                                <div className="space-y-1">
                                    <label className="text-[10px] text-gray-500 uppercase font-bold tracking-wider block">Column</label>
                                    <span className="text-sm font-bold text-white block">{columns.find(c => c.id === selectedCell.colId)?.title}</span>
                                </div>
                                {activeCellData.confidence && (
                                    <span className={`text-[10px] px-2 py-1 rounded-full uppercase font-bold tracking-wide border ${
                                        activeCellData.confidence === 'High' ? 'bg-green-500/10 text-green-400 border-green-500/20' : 
                                        activeCellData.confidence === 'Medium' ? 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'
                                    }`}>
                                        {activeCellData.confidence} Confidence
                                    </span>
                                )}
                            </div>

                            <div className="space-y-1">
                                <label className="text-[10px] text-gray-500 uppercase font-bold tracking-wider block flex items-center gap-1"><Edit3 className="w-3 h-3"/> Extracted Value</label>
                                <textarea 
                                    value={activeCellData.value} 
                                    onChange={(e) => handleCellUpdate(e.target.value)}
                                    className="w-full bg-black/50 border border-white/10 rounded-lg p-3 text-sm text-gray-200 focus:border-blue-500 outline-none resize-none min-h-[80px] leading-relaxed transition-colors"
                                />
                            </div>

                            {activeCellData.quote && (
                                <div className="space-y-1">
                                    <label className="text-[10px] text-gray-500 uppercase font-bold tracking-wider block flex items-center gap-1"><Bot className="w-3 h-3"/> AI Reasoning / Evidence</label>
                                    <div className="bg-white/5 rounded-lg p-3 border border-white/5">
                                        <p className="text-xs text-gray-400 italic leading-relaxed">"{activeCellData.quote}"</p>
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex-1 bg-slate-900 relative overflow-hidden">
                            {activeDoc.type === 'pdf' ? (
                                <PDFViewer file={activeDoc.fileObj} highlights={[activeCellData.quote]} initialScale={0.7} />
                            ) : (
