...
**Precedent documents brought in to learn from redlines are read and never stored — not IndexedDB, not `localStorage`, not the URL.** A "Learn from redlines" session (`docxRedlines.ts`, `pdfRedlineDiff.ts`, `inferPositions.ts`) exists to read a firm's own past negotiated documents, not to archive them alongside the playbook they teach; the UI promises this to the user in plain language, so it has to be true of the implementation, not only of the copy. Held in React state for the session and gone on reload — only a `Changeset` or a published playbook version outlives it.
