...
- **Two live `mount()`s in one test leave two competing global listeners.** `mount()` accumulates and only tears down in its `afterEach`, so a test that mounts twice to compare states has both trees alive — and for a hook that binds to `window` (`useVerifyKeys`), the first tree's listener still fires. Use `mountOnce` and unmount explicitly when a test needs a second tree. Found the hard way: a test that looked like it proved movement stops at the ends was actually being answered by a stale listener.
