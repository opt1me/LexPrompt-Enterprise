...
**Date:** 2026-08-28
**Revised:** 2026-08-28, for three owner decisions — see "Revision" below.
...
**Local development is the same shape.** `docker compose up` brings up web, api, gateway, Postgres and Azurite, with the gateway pointed at any configured provider — a Foundry deployment using developer Entra credentials via `DefaultAzureCredential`, a keyed provider using a secret from a local env file, or a recorded-response stub for offline work. `azd up` provisions the same five things in Azure. The compose network denies `api` egress the same way the Container Apps environment does, so the central claim is exercised in development rather than only asserted in production.

**A no-Azure deployment is a first-class path, and it is not a lesser one.** The owner's reason for making providers pluggable is the firm or individual with no Azure tenant, so the local path runs the *same* gateway binary, the *same* allowlist, the *same* jurisdiction check and the *same* per-call log as the Azure one — see §10.4, which states that as an acceptance condition and refuses to let logging become the thing a local deployment quietly skips. **What the no-Azure path does not yet have is an identity and storage story**, and this design does not pretend otherwise: Postgres is Postgres and blob storage substitutes cleanly (S3-compatible, MinIO, Azurite), but §7's authentication is Entra-only by S10, and a firm with no Azure tenant has no Entra tenant either. Stage 1 — the gateway — is unaffected, because it is the stage that has no accounts yet. Stages 2 onward are, and **§17 Q11 asks the owner what "running it locally" is meant to reach**: the inference path only, or the whole stack. Answering it by assumption is how a design ends up claiming a deployment mode it never specified.
...
- **An allowlist of provider+model pairs.** A request naming a pair not on the list is rejected. Each entry carries its **declared processing jurisdiction**, and the gateway refuses a jurisdiction the operator has not explicitly allowed — at startup and again per call (§10.3, S27). A user still cannot name an arbitrary model; that property is exactly as it was under the Foundry-only design, and only the shape of the entry changed.
...
1. **Each allowlist entry carries a `dataHandling` note** — a short operator-supplied statement of that provider's retention and training posture, with the date it was checked and a link to the provider's terms. It is configuration, so it can be corrected without a release.
...
3. **A stale entry is visible as stale.** A `dataHandling` note older than the firm's review interval renders as needing re-checking rather than as current. An unchecked compliance claim presented as checked is this document's founding failure mode wearing a procurement hat.
...
- **A surfaced warning is enforcement by attentiveness, and this project's entire prior is that attentiveness loses.** Every serious defect on `CLAUDE.md`'s list is something incorrect presented in a way that read as correct. A badge saying "US processing" next to a model in a picker is precisely a thing that gets read past at 17:40 on a deadline.
- **The person who would see the warning cannot give the consent it asks for.** A cross-border transfer of privileged client text is a firm-level decision — DPIA, engagement terms, possibly the client's own instructions. The lawyer picking a model is not the person authorised to take it, and asking them to is worse than not asking: it manufactures a record of a decision nobody had authority to make.
- **The costs are asymmetric, and only one of them is recoverable.** Refuse wrongly and a call fails loudly with a `403` naming the provider, its declared jurisdiction and the allowed set — one configuration change, minutes, nothing lost. Surface wrongly and privileged text has already left the jurisdiction; there is no retry that un-sends it. Given that asymmetry the default must be the loud, cheap failure. This is `CLAUDE.md`'s opening rule applied to a network boundary.
- **Fail closed on *undeclared*, not open.** An allowlist entry with no jurisdiction is treated as disallowed, and the gateway **refuses to start** if any configured entry declares none or declares one outside the allowed set. Undeclared is not permission, and a misconfiguration must not become a system that runs and mostly works. There is no default allow-set: an operator who configures nothing gets a gateway that refuses every call and says exactly why, which is the correct behaviour for a component whose job is to be the only way out.

**Refusing does not replace surfacing; it makes surfacing honest.** Both happen, at different altitudes:

- **To the operator, where the choice is made.** The admin provider screen shows each entry's provider, model, endpoint, jurisdiction, credential mode and `dataHandling` note, and the allowed-jurisdiction set as an explicit, dated configuration value. Adding a provider is a deliberate act with all of that in view.
- **To the user, in the model picker.** Every allowlisted model shows its **provider and jurisdiction, always** — never only the ones outside the UK, and never on hover. **Ruling: the label is unconditional**, because a badge shown only on "bad" entries makes its *absence* carry meaning, and absence carrying meaning is this project's blank-CSV-cell defect exactly. The label is a disclosure, not a warning: by the time a model reaches the picker the operator has already allowed its jurisdiction, so the picker's job is to make sure a lawyer is never surprised by where their client's text went, not to ask them to approve it.
- **To the reader of a report.** The run records what was used (§6.5) and the export can state it. A firm that ran forty reviews on a UK deployment and four on a US one must be able to tell which is which afterwards; a single "processed in the UK" sentence composed from current configuration would be a claim about the wrong runs.

**A firm must never be able to believe it is UK-only while privileged text routes to a US region.** That sentence is the requirement; every bullet above is a mechanism for it, and if a mechanism is trimmed the requirement is what to check against.
...
| The gateway refuses a jurisdiction the operator has not allowed, and refuses to start if misconfigured | **Architectural** | Once, from S27's startup and per-call checks. |
...
**The recommended posture, stated once so a reviewer is not left to infer it:** a firm with Azure infrastructure configures Foundry or Azure OpenAI with managed identity in a UK/EU region, and gets every row above in its strongest form. Every other configuration is supported, logged and honestly described — it is not the same answer, and this document does not let it be presented as one.
...
**2. Who can access it?**
Named members of the firm's Entra tenant who are in one of three mapped security groups. There is no password login, no shared key, no user API key, no anonymous link, and no external sharing. **That sentence is about credentials that grant a *person* access, and it is unconditional.** A provider credential, where the deployment has one, is a machine credential held only by the gateway (§10, §12.0): it authenticates the gateway to a model provider and grants no access to this system, to a document, or to a record. The two are named separately here because a reader who collapses them will either over-claim ("no keys anywhere") or under-claim ("there are keys, so someone could log in with one"), and both are wrong. Access is removed by Entra group removal or account disable: HTTP access ends at the next token refresh (≤ 60 minutes) and WebSocket connections for a disabled user are closed immediately by the server. Infrastructure access (DBA, blob) is Azure RBAC with PIM approval and is recorded in Azure activity logs, outside the application's control.
...
**In every deployment this design specifies:** Microsoft, for the hosting, storage and identity — Azure Container Apps, Azure Database for PostgreSQL, Azure Blob Storage, Microsoft Entra ID, Azure Monitor. Client data at rest and the identity layer are Microsoft-only regardless of which model provider is configured, and **the multi-provider revision changes nothing about either.** (A fully self-hosted deployment would substitute the storage half and would need an identity answer this design does not yet have — §17 Q11. It is named here so the qualifier above is read as deliberate rather than as an oversight.)
...
- **What is configured** — the allowlist, listing each provider+model pair with its endpoint, declared jurisdiction, credential mode and `dataHandling` note (§10.1), plus the allowed-jurisdiction set. This is what the firm has *permitted*.
- **What was actually used** — the gateway call log, aggregated by provider and jurisdiction over any period. This is what the firm actually *did*, and it is the half a reviewer cannot get from a configuration file. "We allow only UK deployments" and "every one of the 12,400 calls last quarter went to a UK deployment" are different statements, and the second is the one that is evidence.

Both are exportable by an admin, together, as the subprocessor report.

**On jurisdiction, plainly.** OpenRouter, Anthropic direct and OpenAI direct are **US-processing**, and OpenRouter additionally routes onward to a provider the operator did not individually select. Foundry and Azure OpenAI can be pinned to a UK or EU region. **A firm that wants a UK/EU-only answer to this question configures only UK/EU-pinned entries and sets its allowed-jurisdiction set accordingly; the gateway then refuses anything else (§10.3, S27), including a US entry added later by mistake.** Where a desired model has no UK/EU deployment, it is not allowlisted and the app cannot use it — the same capability limit as before, now a deliberate configuration rather than an assumption baked into the design. **The owner's own stated constraint is UK/EU with US processing avoided**, and that is what the shipped deployment template configures; a different operator making a different, recorded choice is the thing this revision makes possible, not a loophole in this answer.
...
| `jurisdiction` | a pair whose declared jurisdiction is outside the allowed set is refused with a `403` naming provider, jurisdiction and allowed set, and **no request reaches the provider**; an entry declaring no jurisdiction is refused; the gateway **refuses to start** on either misconfiguration, and on a missing log sink; the picker labels every model with provider and jurisdiction, including UK ones; a run row stores the jurisdiction the gateway returned and does not re-derive it from configuration |
...
- **S15 (amended 2026-08-28). Models are an admin-configured allowlist of provider+model pairs, each declaring the jurisdiction its processing happens in, and the operator sets an explicit allowed-jurisdiction set the gateway enforces. A user still cannot enter a model id.** *Cost if wrong: a model whose provider or region the firm has not allowed is unavailable, and adding one is a configuration change plus a Risk sign-off — which is the point, and is unchanged. The property that a user cannot name an arbitrary egress destination is the half that must not move; the shape of the allowlist entry is the half that did.*
  - *Amended, 2026-08-28: S15 originally read "an admin-configured allowlist of **Foundry deployments**, region-pinned to UK/EU", which assumed one provider and inferred jurisdiction from an Azure region name. With five possible providers, region is no longer a property the design can read off the entry, so each entry **declares** its jurisdiction and the allowed set is explicit configuration rather than a hard-coded UK/EU rule (S26, S27). The owner's own constraint — UK/EU, US processing avoided — becomes what the shipped deployment template configures rather than what the code assumes, and it is enforced exactly as strictly. Recorded rather than deleted: a reader must be able to see that "UK/EU-pinned" moved from an assumption to a configured, enforced, per-deployment fact.*
...
- **S26 (2026-08-28). Every allowlist entry declares its processing jurisdiction and a dated `dataHandling` note. Provider, model and jurisdiction are returned on every gateway response, written on every call-log line, and stored on the `run` row as a snapshot rather than re-derived from current configuration. The per-call log is not optional in any deployment, and the gateway refuses to start with no log sink configured.** *Cost if wrong: three columns, three log fields and one startup check. Without the declaration, "where is privileged text processed?" stops being answerable the moment the destination becomes configuration — and without the snapshot, a firm that later changes its allowlist silently rewrites the answer for every run it already performed, which is `playbookSnapshot`'s failure applied to the one fact a data-protection question turns on. Without the logging floor, a local deployment becomes the one that quietly skips the record, and it would be the deployment least likely to have anyone checking.*
- **S27 (2026-08-28). The gateway refuses a jurisdiction the operator has not explicitly allowed — it does not merely surface it. An entry declaring no jurisdiction is refused; there is no default allow-set; the gateway refuses to start on either misconfiguration. Surfacing happens as well, at two altitudes: the operator sees jurisdiction where the choice is made, and the model picker labels every model with its provider and jurisdiction unconditionally, never only the non-UK ones.** *Cost if wrong: a misconfiguration fails loudly with a `403` naming the provider, its jurisdiction and the allowed set — one config change, minutes, nothing lost. The opposite error cannot be undone: privileged client text that has crossed a border cannot be un-sent. The asymmetry is the whole argument, and it is `CLAUDE.md`'s opening rule at a network boundary. A surfaced-only warning would also ask the wrong person for the consent — a lawyer picking a model at 17:40 cannot authorise a cross-border transfer of a client's privileged text, and a record of them appearing to is worse than no record. Labelling only the non-UK models would make the **absence** of a label carry meaning, which is the blank-CSV-cell defect exactly.*
...
4. **Provider choice and provider-side retention (§10.1) — widened by the multi-provider decision, and it is now two questions.** *(a)* **Which providers will the firm's own deployment actually configure**, and what is its allowed-jurisdiction set? The design's answer is UK/EU with US processing avoided, taken from the owner's stated constraint and shipped as the deployment template's configuration — but it is now a *choice a deployment makes* rather than something the code assumes, so the owner must state it as a decision and not leave it to a default. Configuring OpenAI, Anthropic or OpenRouter directly means US processing (§12 Q5), and the gateway will refuse them unless the allowed set says otherwise. *(b)* For each provider actually configured, what is its retention and training posture, and is it accepted or mitigated? For Foundry that is the modified-abuse-monitoring exemption — apply before go-live, or accept and disclose up-to-30-day retention with possible human review. For the others it is that provider's current terms, read at implementation time. **Affects §10.1, §12 Q4, §12 Q5 and the disclosure the app itself shows.**
...
11. **NEW, 2026-08-28 — what does "running it locally" mean, and how far does it reach?** The owner's reason for pluggable providers is *"smaller firms or individuals running it locally who won't have Azure infrastructure"*, and **the gateway now serves that fully. The rest of the design does not yet, and this document will not pretend it does.** Postgres is Postgres and blob storage substitutes cleanly (S3-compatible, MinIO, Azurite), so storage is a packaging exercise. **Identity is not**: §7 is Entra-only by S10, and a firm with no Azure tenant has no Entra tenant either. Stage 1 is unaffected, because it has no accounts. Stages 2 onward are. So: is the local path (a) the *inference* path only, with the deployed multi-user app remaining Azure/Entra — which is what this document specifies today and is a coherent answer; (b) a genuine single-user local build with no accounts, effectively today's app with a gateway in front, which is cheap but reopens R1; or (c) a full local deployment with a non-Entra identity provider, which reverses S10's "no SSO beyond Entra" and adds a credential store the design deliberately refused to build? **Affects §5, §7, §13 and S10.** *My recommendation is (a) for now and (b) as an explicit, labelled build target if individuals are a real audience — (c) is a subsystem, not a configuration, and it should be decided as one rather than arrived at.*
...
**A jurisdiction guarantee that lives in configuration can be true of the design and false of an environment.** The gateway enforces the operator's allowed set, so the code cannot be wrong about it — but the *operator* can, by allowlisting an entry whose declared jurisdiction is copied from another entry, or by leaving a `dataHandling` note that was true two years ago. The declaration is only as good as the person who typed it. §10.1's staleness rendering and §12 Q5's "what was actually used" call-log report are the mitigations, and the second is the real one: configuration says what a firm permitted, the call log says what it did, and only the second is evidence.
...
| 1 — gateway + Entra + inference proxy + provider adapters | ~1.25–1.5 (was ~1 before the multi-provider revision) |
