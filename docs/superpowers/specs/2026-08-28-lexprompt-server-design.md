...
1. **The declared jurisdiction set has no default at all** — not `UK,EU`, not anything. A default encodes an assumption about one firm's contracts. The gateway **refuses to start** when the set is unset, exactly as it refuses to start on an undeclared entry. (The gateway's audit log carries the same fail-closed spirit by a stronger route: there is no log-sink configuration to leave unset in the first place — §10.5.) Fail-closed is unchanged and is now *stronger*: unconfigured is an error rather than a guess. The deployment template carries `UK,EU` as a **commented example with its reasoning**, never as a silent default (§10.3, §12 Q5, S27).
...
| 8 | Gateway log **collector** — not the sink itself, which never varies | a collector shipped in the compose file | Azure Monitor | Configuration, and only of the collector. The gateway writes the same JSON lines to stdout in both, with no log-sink configuration key in either — there is nothing here for a deployer to point elsewhere (§10.5). |
...
- The **per-call log** applies, with the same fields including provider and jurisdiction. The gateway holds no database credential (§5), so it writes structured JSON lines to stdout in every deployment — collected by Azure Monitor in Azure, and by a collector the compose file ships in a local one. **There is no log-sink configuration key, in any deployment — not an environment variable, not an admin setting, nothing a deployer can point elsewhere or leave unset.** *(Revised, 2026-08-28: this was originally a startup refusal — "the gateway refuses to start with no log sink configured" — for the same reason it refuses to start with no declared jurisdiction set or on an undeclared entry. A refusal is a check on a configuration value, which means a configuration value has to exist first; the implementation is stronger than that and has no such value to check. Writing to stdout is not a default that can be overridden or a startup precondition that can be satisfied by pointing somewhere else — it is the only thing the audit sink does, unconditionally, in every deployment. With nothing to configure, there is nothing to misconfigure, which removes the failure mode instead of detecting it after the fact.)* The reasoning that justified the refusal still holds and now justifies the stronger mechanism instead: a deployment able to configure its way out of logging would teach its operator to expect a gateway that sometimes does not, and that expectation would eventually be carried into a deployment where it mattered — so no deployment is given the means to.
...
**A separate Stage 1 definition-of-done check holds the audit sink's absence of configuration, rather than a fifth suite asserting a startup refusal that does not exist.** *(Revised, 2026-08-28: previously the `jurisdiction` row above also asserted "the gateway refuses to start ... on a missing log sink", grouped with the jurisdiction misconfigurations because both were startup checks. The audit sink has no configuration to check at startup, so there is nothing for that assertion to exercise; the check that replaces it is structural instead.)* It asserts `apps/gateway/src/config.ts` declares no key matching `GATEWAY_*LOG*` or `*SINK*` (a pattern search, not a lookup of a name someone remembered to add) and that `apps/gateway/src/audit.ts` never reads `process.env` at all — so a log-sink configuration surface fails the build the day anyone adds one, precisely because none exists to add correctly (§18 item 2).
...
- **S26 (2026-08-28, amended twice, most recently 2026-08-28). Every allowlist entry declares its processing jurisdiction and carries a dated `dataHandling` note recording the terms the operator agreed with that provider. Provider, model and jurisdiction are returned on every gateway response, written on every call-log line, and stored on the `run` row as a snapshot rather than re-derived from current configuration. The per-call log is not optional in any deployment: the gateway writes it to stdout unconditionally, and there is no log-sink configuration key for any deployment to leave unset or point elsewhere.** *Cost if wrong: three columns, three log fields, and a configuration surface kept permanently empty. Without the declaration, "where is privileged text processed?" stops being answerable the moment the destination becomes configuration — and without the snapshot, a firm that later changes its allowlist silently rewrites the answer for every run it already performed, which is `playbookSnapshot`'s failure applied to the one fact a data-protection question turns on. Without the logging floor, a local deployment becomes the one that quietly skips the record, and it would be the deployment least likely to have anyone checking.*
  ...
  - *Amended, 2026-08-28 (implementation, on the Stage 1 gateway plan): the logging floor was originally a startup check — "the gateway refuses to start with no log sink configured" — mirroring the jurisdiction refusals below it. Two independent task implementers hit a contradiction: the plan makes `JsonlAuditSink` write JSON lines to stdout unconditionally, with no log-sink configuration key at all, and a `stage1DoD` test forbids one from ever being added (no `GATEWAY_*LOG*` or `*SINK*` key in `apps/gateway/src/config.ts`). A refusal presupposes a configuration value to refuse on; this mechanism has none. **The logging floor is unchanged — the per-call log is still not optional in any deployment — but the guarantee is now structural rather than a startup check: with no log-sink configuration surface, no deployment can be configured not to log, which is strictly stronger than validating that one wasn't.** The startup-refusal form is retired, not deleted from memory, so a reader of an earlier draft can see it was replaced deliberately.*
...
2. **Stage 1:** no user-supplied key exists anywhere in the codebase, in any browser, or in any request — searched for, not assumed; every model call goes through the gateway; **no credential of any kind exists outside the gateway process**, asserted by a test over `apps/web` and `apps/api`; every call is logged with its purpose, provider, model and jurisdiction, and with no prompt or completion content, both asserted by tests; the allowlist refuses an unknown provider+model pair and the gateway refuses a jurisdiction outside the declared set without the request reaching the provider; **the gateway refuses to start with no jurisdiction set declared** — searched for, not assumed: no default value appears anywhere in configuration loading — and likewise on an undeclared entry; **and, in place of a startup refusal for the audit log: no log-sink configuration key exists anywhere in the gateway's configuration surface** — searched for, not assumed, by a pattern match over `apps/gateway/src/config.ts` for anything matching `GATEWAY_*LOG*` or `*SINK*`, and by asserting `apps/gateway/src/audit.ts` never reads `process.env` — so the audit line's presence in every deployment is guaranteed by having nothing to misconfigure, rather than by a startup check on something that could be *(revised, 2026-08-28: this line previously read "the gateway refuses to start with no log sink configured"; the implementation has no log-sink configuration to refuse on, by design, and the check above is what actually guards the property)*; the adapter-boundary test passes and the stream conformance suite passes **for every registered adapter**, including the byte-for-byte streamed-equals-non-streamed case on a CRLF fixture.
